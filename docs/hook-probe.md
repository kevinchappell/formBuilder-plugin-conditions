...
| Wildcard `typeUserEvents.onclone` | Pass | The wildcard `onclone` callback receives the real clone and updates its `.fld-conditionId` value to `email-visible-copy`. |
